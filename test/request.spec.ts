...
describe('requests', () => {
  ...
  test('should treat method value as lowercase string', done => {
    ...
    getAjaxRequest().then(request => {
      request.respondWith({
        // 模拟响应成功，并且设置响应状态码为 200 ，响应成功后执行上面的 then 里面的逻辑
        status: 200
      })
    })
  })
  ...
  test('should reject on network errors', done => {
    ...
    function next(reason: AxiosResponse | AxiosError) {
      ...
      expect(reason.request).toEqual(expect.any(XMLHttpRequest)) // expect.any(constructor) -> 它表示匹配任意由 constructor 创建的对象实例。
  ...
  test('should reject when request timeout', done => {
    ...
    getAjaxRequest().then(request => {
      // @ts-ignore
      request.eventBus.trigger('timeout') // 这又是 jasmine 的插件不兼容 jest 框架的 hack 处理，这样子处理可以触发 timeout 事件超时状况。就会执行上面 catch 逻辑
